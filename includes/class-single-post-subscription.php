...
class Single_Post_Subscription {
	...
	public static function manage_paywall_restriction() {
		...
		if ( isset( $_COOKIE[ $cookie_name ] ) ) {
			...
			remove_filter( 'the_posts', array( $membership_instance, 'exclude_restricted_content_comments' ), 999, 2 );
			remove_filter( 'pre_get_comments', array( $membership_instance, 'exclude_restricted_comments' ), 999 );
			...
			remove_filter( 'posts_clauses', array( $membership_instance, 'handle_posts_clauses' ), 999, 2 );
			remove_filter( 'get_terms_args', array( $membership_instance, 'handle_get_terms_args' ), 999, 2 );
			remove_filter( 'terms_clauses', array( $membership_instance, 'handle_terms_clauses' ), 999 );
			...
			add_filter( 'the_posts', array( $membership_instance, 'exclude_restricted_content_comments' ), 999, 2 );
			add_filter( 'pre_get_comments', array( $membership_instance, 'exclude_restricted_comments' ), 999 );
			...
			add_filter( 'posts_clauses', array( $membership_instance, 'handle_posts_clauses' ), 999, 2 );
			add_filter( 'get_terms_args', array( $membership_instance, 'handle_get_terms_args' ), 999, 2 );
			add_filter( 'terms_clauses', array( $membership_instance, 'handle_terms_clauses' ), 999 );
		}
	}
}
